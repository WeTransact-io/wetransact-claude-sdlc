...
- Apply structured debugging approach if stuck
...
Use `integration-test-executor` subagent to run tests (or `e2e-test-executor` for UI/browser testing).
...
- Update docs if needed via `documentation-generator`
...
| 4 | `integration-test-executor` subagent (or `e2e-test-executor` for UI/browser) |
| 5 | `code-reviewer` subagent |
| 6 | `git-manager`, `documentation-generator` subagents |
