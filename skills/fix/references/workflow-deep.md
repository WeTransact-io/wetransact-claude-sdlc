...
Use WebSearch/WebFetch for external knowledge.
...
- Evaluate multiple approaches based on research findings
- Consider trade-offs
- Use `AskUserQuestion` for user input on preferred direction
...
Use `implementation-planner` subagent to create implementation plan.
...
Use `integration-test-executor` subagent (or `e2e-test-executor` for UI/browser testing).
...
- Update plan/roadmap status directly (edit plan.md + phase files)
- Use `documentation-generator` subagent for documentation
...
| 2 | WebSearch/WebFetch (runs parallel with step 1) |
| 3 | Evaluate approaches, use `AskUserQuestion` for direction |
| 4 | `implementation-planner` |
| 5 | parallel `Bash` |
| 6 | `integration-test-executor` (or `e2e-test-executor` for UI/browser testing) |
| 7 | `code-reviewer` |
| 8 | `documentation-generator`, `Bash`, direct plan status update |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `playwright-cli` skill to verify.
