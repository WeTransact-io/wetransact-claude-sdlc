 ...
 * H7 Fix: Don't expose passphrase in plaintext - subagents inherit trust from parent
 */
function buildTrustVerification(config) {
  if (!config.trust?.enabled) return [];
  return [
    ``,
    `## Trust Verification`,
    `This subagent inherits trust from parent session`
  ];
}
