...
### Parallel (→ multiple `senior-backend-engineer` + `senior-frontend-engineer` agents) — Use Task Trees
...
**Task usage:** Create separate task trees per independent issue. Spawn `senior-backend-engineer` + `senior-frontend-engineer` agents per tree (in parallel). Agents coordinate via `TaskUpdate`/`TaskList`. See `references/task-orchestration.md`.
