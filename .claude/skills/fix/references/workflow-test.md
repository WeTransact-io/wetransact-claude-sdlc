...
Use `integration-test-executor` agent. Fix all syntax errors before running tests.
...
Use `implementation-planner` agent for fix strategy.
...
Use `integration-test-executor` agent. If tests still fail → keep T5 `in_progress`, loop back to Step 2.
