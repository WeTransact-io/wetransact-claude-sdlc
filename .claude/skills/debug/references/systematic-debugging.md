...
## System-Level Investigation (Incidents, Multi-Component Failures)

When issue spans services/infra (not just a code bug), use this 5-step process:

1. **Assess** — Collect symptoms, identify affected components, check recent deployments (`gh run list`, `git log --oneline -20`), determine severity
2. **Collect** — Gather logs (server, CI/CD, app), database state, system metrics, external dependency status
3. **Analyze** — Build timeline across sources, identify patterns, trace request flow, map dependency chain
4. **Root Cause** — Rank hypotheses by evidence, test each minimally, validate with logs/metrics, document trigger→symptom chain
5. **Solution** — Immediate fix (restore service) → Root cause fix → Preventive measures → Verification plan

When investigation narrows to specific code → switch to Phase 1-4 above.
