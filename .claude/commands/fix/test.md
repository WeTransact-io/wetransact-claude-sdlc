...
## Workflow:
1. Use `debugger` subagent to compile the code and fix all syntax errors if any.
2. Use `integration-test-executor` subagent to run the tests and report back to main agent.
...
6. Use `integration-test-executor` subagent to test the fix and make sure it works, then report back to main agent.
