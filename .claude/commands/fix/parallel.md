---
description: Analyze & fix issues with parallel engineer agents
...
- Use Explore subagents with Glob/Grep to find related files
...
- Launch multiple `senior-backend-engineer` / `senior-frontend-engineer` agents in PARALLEL for independent fixes
...
**Example:** Fix 1 (auth) + Fix 2 (payments) + Fix 3 (UI) → Launch 3 engineer agents (senior-backend-engineer / senior-frontend-engineer) → Wait → Fix 4 (integration) sequential
