...
## Workflow
1. If the user provides screenshots or videos, describe as detailed as possible the issue, make sure developers can predict the root causes easily based on the description.
2. Use `debugger` subagent to find the root cause of the issues and report back to main agent.
3. Activate `debug` skill to tackle the issues.
4. Start implementing the fix based the reports and solutions.
5. Use `integration-test-executor` subagent to test the fix and make sure it works, then report back to main agent.
